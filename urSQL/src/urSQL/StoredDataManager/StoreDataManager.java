...
public class StoreDataManager {
	...
	public StoreDataManager(){
		...
		if(!databases_dir.exists()){
			...
			//si no se pudo crear envï¿½a un mensaje de error
			if(!result){
	 ...
	 * 
	 * @param metadata informaciï¿½n que uno le importa 
	 * 
	 ...
	 * 
	 * @param linkedList Informaciï¿½n de las columnas de la tabla 
	 * 
	 * @param data informaciï¿½n que se va a guardar en la tabla
	 * 
	...
	/**
	 * Pasa a bytes la informaciï¿½n que se quiere almacenar
	 * en la tabla.
	 * 
	 * @param vec Vector con el pares de string donde 
	 * el primer objeto es el tipo del objeto, y el 
	 * segundo es la informaciï¿½n a almacenar
	 * 
	 * @return un arreglo de bytes con toda la informaciï¿½n
	 */
	 ...
	 * para ser leidos cuando son extraï¿½dos
	 * 
	 * @param type Tipo de informaciï¿½n que se almacena dentro
	 * en el registro
	 * 
	 * @param data informaciï¿½n que se almacena en el registro
	 * 
	 ...
	 * los subregistros de informaciï¿½n con su debido enccabezado
	 * 
	 * @param bytes vector con todos los arreglos de bytes que 
	 * van a ser concatenados.
	 * 
	 * @return un arreglo de bytes cono resultado de la concatenaciï¿½n
	 * de todos los arreglos de bytes
	 */
	private byte[] concatenateByteArray(Vector<byte[]> bytes){
		//tamaï¿½o total del arreglo de bytes
		int total_size = 0;

		for (int i = 0; i < bytes.size(); i++) {
			total_size += bytes.get(i).length;
		}
		//se crea el arreglo con el tamaï¿½o total
	 ...
	 * 
	 * @param type tipo de la informaciï¿½n que se va a guardar
	 * en el registro
	 * 
	 * @param size cantidad de bytes que va a ocupar el registro
	 * 
	 * @param data informaciï¿½n del registro 
	 * 
	 * @return arreglo de bytes donde se contiene la informacion,
	 * el tamaï¿½o y el tipo de informacion guardada
	 */
	private byte[] makeRegister(byte type, int size, byte[] data){
		...
		reg[0] = type;
		//se agrega el tamaï¿½o
		byte[] by_size = short2bytes((short)size);
		reg[1] = by_size[0];
		reg[2] = by_size[1];
		//se agrega la informaciï¿½n
						...
						//si existen
							...
							try {
								...
								if(!tree.ContainsKey(key)){
									...
									//se obtiene la informaciï¿½n de las columnas
									byte[] b_pk_index = tree.get(PK_INDEX);
									int pk_index = (int)ByteBuffer.wrap(b_pk_index).getShort();
									...
									if(key.compareTo(tmp_key)!=0){
										...
										//Vector con tipos e informaciï¿½n
	 ...
	 * 
	 * @param metadata Informaciï¿½n de la tabla que esta almacenada
	 *  
	 * @param data informaicï¿½n que se va a escribir
	 * 
